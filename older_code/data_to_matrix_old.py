...
def merge_all_metadata(folder_path='./raw_data', out_folder_path='./raw_csv_data'):
    raw_files = os.listdir(folder_path)  # list all raw files
    # print(raw_files)
    raw_files = list(filter(lambda x: '_metadata_filtered.csv' in x, raw_files))  # filter files which are barcodes files
    # print(raw_files)
